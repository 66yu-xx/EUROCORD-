...
function warehouseAlertsPage() {
  ...
  const feedbackMessages = alertRows.length || recheckRows.length ? [alertRows.length ? '存在库存预警物料，计划和采购可优先关注这些物料对后续交期判断的影响。' : '', recheckRows.length ? '存在待人工核对的库存项目，相关库存数字在订单判断前可进一步确认。' : ''].filter(Boolean) : ['当前没有明显库存预警或仓库复查提示，库存数据仅作为当前只读参考。'];

  return `${skeletonNotice('库存预警与仓库反馈（只读）', '这是仓库视角的只读库存风险视图，按“库存概览、预警列表、复查提示、计划 / 采购反馈”顺序阅读。当前不修改库存、不保存复查结果、不生成采购单。')}
