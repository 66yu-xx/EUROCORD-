...
function warehouseAlertsPage() {
  ...
  const alertTable = alertRows.length ? `<div class="table-wrap"><table><thead><tr><th>物料编码</th><th>物料名称</th><th>当前库存</th><th>安全库存</th><th>预警类型</th><th>原因</th></tr></thead><tbody>${alertRows.map((row) => { const isZero = row.stockQty <= 0; return `<tr><td><strong class="code">${row.material.code}</strong></td><td>${row.material.name}</td><td><strong>${format(row.stockQty)}</strong> ${row.material.unit}</td><td>${format(row.safetyStock)} ${row.material.unit}</td><td><span class="soft-tag">${isZero ? '库存为 0' : '低于安全库存'}</span></td><td>${isZero ? '当前库存小于等于 0，需要仓库优先复查库存数字。' : '当前库存低于安全库存，建议仓库提醒计划 / 采购关注。'}</td></tr>`; }).join('')}</tbody></table></div>` : empty('暂无库存预警', '当前没有库存为 0 或低于安全库存的物料。');
  const recheckRows = inventoryRows.filter((row) => row.stockQty <= 0 || (row.stockQty > 0 && row.stockQty < row.safetyStock) || row.safetyStock <= 0);
  const recheckTable = recheckRows.length ? `<div class="table-wrap"><table><thead><tr><th>物料编码</th><th>物料名称</th><th>当前库存</th><th>安全库存</th><th>复查类型</th><th>复查原因</th></tr></thead><tbody>${recheckRows.map((row) => { const type = row.stockQty <= 0 ? '确认无库存' : row.safetyStock <= 0 ? '补充安全库存设置' : '确认可用数量'; const reason = row.stockQty <= 0 ? '当前库存小于等于 0，建议仓库确认是否确实无可用库存。' : row.safetyStock <= 0 ? '安全库存未维护或为 0，建议补充预警基准。' : '当前库存低于安全库存，建议复查账面数量和现场可用数量。'; return `<tr><td><strong class="code">${row.material.code}</strong></td><td>${row.material.name}</td><td><strong>${format(row.stockQty)}</strong> ${row.material.unit}</td><td>${format(row.safetyStock)} ${row.material.unit}</td><td><span class="soft-tag">${type}</span></td><td>${reason}</td></tr>`; }).join('')}</tbody></table></div>` : empty('暂无复查建议', '当前没有需要仓库复查的库存项目。');
  const feedbackMessages = alertRows.length || recheckRows.length ? [alertRows.length ? '存在库存预警物料，计划和采购应优先关注这些物料对后续交期判断的影响。' : '', recheckRows.length ? '存在建议复查的库存项目，相关库存数字建议仓库确认后再用于订单判断。' : ''].filter(Boolean) : ['当前没有明显库存预警或仓库复查建议，库存数据可作为当前只读参考。'];
  ...
  return `${skeletonNotice('库存预警与仓库反馈', '这是仓库角色使用的只读库存风险视图，用于查看库存预警、复查建议，以及对计划 / 采购的反馈提示。当前不修改库存、不保存复查结果、不生成采购单。')}
    ...
    <article class="panel table-panel" style="margin-bottom:18px"><div class="panel-head"><div><span class="kicker">WAREHOUSE ALERTS</span><h3>库存预警列表</h3></div><span class="version">${alertRows.length} 项 · 只读</span></div><div class="placeholder-copy"><p>未维护安全库存或安全库存为 0 的物料暂不纳入低库存预警；本列表只显示库存为 0 和低于安全库存的物料。</p></div>${alertTable}</article>
    <div class="document-shell"><article class="panel table-panel"><div class="panel-head"><div><span class="kicker">RECHECK NOTES</span><h3>仓库复查建议</h3></div><span class="version">${recheckRows.length} 项 · 不保存结果</span></div><div class="placeholder-copy"><p>以下内容只是复查建议，不会保存复查结果、修改库存数据或生成采购单。</p></div>${recheckTable}</article>
    <article class="panel workflow-panel"><span class="kicker">FEEDBACK ONLY</span><h3>对计划 / 采购的反馈提示</h3><div class="workflow-steps">${feedbackMessages.map((message) => `<span>${message}</span>`).join('')}</div><p>以上提示只是仓库视角反馈，不会修改 Phase 2C 的采购建议，不会生成采购单，也不会保存任何反馈结果。</p></article></div>`;
}
