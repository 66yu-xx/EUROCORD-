...
- Phase 5 当前稳定基线为 `751fd73 clarify warehouse feedback role flow`，分支为 `lufuta-material-system-lite`，测试基线为 47/47 pass。
- Phase 5 是仓库侧库存预警反馈的只读演示冻结；仓库反馈是库存可信度信号，不是真实库存操作系统。
- 仓库只反馈现场库存可信度，不直接修改系统库存，不生成采购单，不确认采购建议，不影响 MRP 核心计算结果。
- 老板、计划、采购、仓库可以阅读仓库反馈状态和四角色信息流说明，但这些说明只用于人工复核提醒。
- Phase 5 不得保存反馈、提交反馈、写入 localStorage、改变库存、影响 MRP 核心计算、改变交期风险计算、改变采购建议、生成采购单、确认采购建议、新增权限系统或新增财务金额。
- 未来如需真实仓库反馈记录、盘点异常记录、跨页面联动、权限或审核流，必须由 Product Owner 单独确认后重新定义范围。
